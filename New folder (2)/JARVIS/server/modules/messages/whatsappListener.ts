...
router.post("/webhook/whatsapp", async (request: Request, response: Response) => {
  const from = String(request.body?.from ?? "").trim();
  const text = String(request.body?.text ?? "").trim();
  const isGroup = Boolean(request.body?.group || request.body?.group_id);
  if (!from || !text || isGroup) {
    response.status(400).json({ error: "WhatsApp personal chat payload required" });
    return;
  }

  const messageId = await queueMessage(from, "whatsapp", text);
